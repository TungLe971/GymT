...
const Header = () => {
  ...
  const fetchAllNotificationData = async () => {
    ...
    try {
      const response = await requestApi('/notifications?status_n=1', 'GET');
      const totalPages = response.data.lastPage;
      for (let page = 1; page <= totalPages; page++) {
        const pageResponse = await requestApi(`/notifications?status_n=1&page=${page}`, 'GET');
        const filteredNotifications = pageResponse.data.data.filter((notification) => notification.status_n === 1);
        allNotifications = [...allNotifications, ...filteredNotifications];
      }
  ...
  const fetchAllData = async () => {
    try {
      ...
      const notifications = await fetchAllNotificationData();
      setNotifications(notifications);
      setHeaderData({ totalNotification: notifications.length });
      setLoading(false);
  ...
  const menu = (
    <Menu>
      <Menu.Item key="1" onClick={handleProfile} icon={<UserOutlined />}>
        Profile
      </Menu.Item>
      ...
      <div className="left-content">
        <Link to="/">
          <img
            ...
            style={{ height: 40 }}
          />
          <span style={{ color: 'rgb(65 48 143 / 91%)' }}>GymT</span>
        </Link>
      </div>
      <div className="center-content">
        <Input prefix={<SearchOutlined />} placeholder="Tìm kiếm..." style={{ width: 200 }} />
      </div>
      <div className="right-content">
        <Button className="btn-rgt" type="text" onClick={handleRegister}>
          Register
        </Button>
        <FloatButton
          loading={loading.toString()}
          onClick={showDrawer}
          badge={{ count: headerData.totalNotification, overflowCount: 99 }}
          icon={<BellOutlined />}
          type="default"
